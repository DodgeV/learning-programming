...
* 基本型`char`等价于`signed char`1 字节，-128 到 127 或 0 到 255，`int`2 或 4 字节
...
* 2. <u>枚举类型</u>，也是算术类型，被用来定义在程序中只能赋予其一定的离散整数值的变量。 
> + 
> + 
> + 
* 3. 空类型(void类型)，类型说明符为void，表明没有可用返回的值。
* 4. 派生类型，包括指针类型、数组类型、结构体类型、共用体类型和函数类型。
