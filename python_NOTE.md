...
d = a.copy() # 浅拷贝
```
* 只要是拷贝，地址就会变，深拷贝浅拷贝的区别在于内部元素的地址是否变
...
```
* 用`copy.copy`实现浅拷贝时，
* 若拷贝可变类型比如列表时，地址会变，内部元素地址不变
* 若是拷贝不可变类型比如元组，则地址不变，仅仅是多贴了1个标签(指向)
...
```
* 深拷贝可变类型时地址会变，简单说就是深拷贝不仅拷贝了数据，还把数据中的引用的源数据也拷了1份
...
* 拷元组时，若元组里面只有基本的不可变类型数据，则`copy`和`deepcopy`都是引用，地址不变
* 若元组里面有引用其他可变类型数据比如列表，`copy`时依然不拷贝，只是引用，但是`deepcopy`时会将内部引用拷1份，造成地址改变(拷贝其他拥有不可变类型的数据，即使元组时顶层，结果一样)
```python
c = (b,a)
print(id(c))  # 140336493882056
d = copy.copy(c)
print(id(d))  # 140336493882056
e = copy.deepcopy(c)
print(id(e))  # 140336493913032
b.append(55)
print(c) # ([22, 33, 55], [11, 22])
print(d) # ([22, 33, 55], [11, 22])
print(e) # ([22, 33], [11, 22])
```
```python
b.append('you')
...
dict3 = dict2 # 直接赋值会受原字典改变的影响 因为不会改变id地址
dict3 = dict2.copy() # 浅拷贝会改变id地址,原字典的键的改变并不影响dict3
dict2['b'].append(68) # 但原字典内部键对应的值的改变会影响
print(dict2 == dict3) # True
dict2.clear() # 删除所有的键值对 改变其中之一只会影响地址相同的
...
nums = [11,22]
test_nums(nums)
print(nums)  # [11,22,33]  全局变量会被修改
test_nums(copy.deepcopy(nums)) # 若不想修改原数据，则可以传入拷贝
```
```python
num = {}
...
```
### 多值不定长参数 
* `*`告诉编译器在传入参数时，多传的通通给`*args`以元组身份保存,`**`用来接收多传的关键字参数作为字典
```python
...
def test1(a, b, *args, **kwargs):
    ...
    # test(a, b, args, kwargs)  # 如果不加声明，会将后面都当成元组的元素传入 相当于test(11, 22, (33, 44, 55, 66), {"name":"laowang", "age":18})
    # test(a, b, *args, kwargs)  # 相当于test(11, 22, 33, 44, 55, 66, {"name":"laowang", "age":18})
    test(a, b, *args, **kwargs)  # 需进行拆包 相当于test(11, 22, 33, 44, 55, 66, name="laowang", age=18)

test1(1,2,1,2,3,4,name="langfang",age=28)
test1(1,2,*(1,2,3,4),**{"name":"langfang","age":28}) # 两句执行结果一样 
```
结果如下
```
1
...
* 3. OOP相比于面向过程是一个更大的封装，封装是为了数据的隐蔽性，继承是为了子类调用父类的属性或方法更方便
...
* `__getslice__(self,i,j)`      定义获取列表中指定步长的元素，相当于 self[i:j]
* `__setslice__(self,i,j,sequence)`      定义设置列表中指定步长的元素，相当于 self[i:j] = sequence
* `__delslice__(self,i,j)`      定义删除列表中指定步长的元素，相当于 del self[i:j]
...
obj = Foo()
result = obj['k1']  # 自动触动执行 __getitem__ 字典的键
obj['k2'] = 'laowang' # 自动触动执行 __setitem__ 设置字典的键值对
del obj['k1'] # 自动触动执行 __delitem__
```
```python
class Foo(object):
...
obj = Foo()
obj[-1:1]  # 自动触动执行 __getslice__ 列表的切片
obj[0:1] = [11,33] # 自动触动执行 __setslice__ 
del obj[0:2] # 自动触动执行 __delslice__
```
```python
class MyTimer():   
    def __init__(self):
...
import random
class Person(object):    
    population = 0 # population是类属性,是所有实例共用的,下面的name是实例属性,是实例私有的
    name_list = []
...
print(A.__module__) # 输出类对象所在的模块
print(A.__class__) # 输出类
A() # 直接在实例对象后面加一对括号调用__call__方法
...
# 与None进行比较时，最好使用is     
A.z = 10 # 加个属性,此为动态绑定
print(A._Person__age) # 在私有属性前加下划线类名可以访问私有属性，因为python的私有是伪私有，并不是真正的隐藏私有，只是将属性的名字改了
...
class Programmer(Person):
    ...
    def get_info(self):
        """因为python中不允许出现同一变量名,变量名指向所有的函数/方法,后定义的变量名会覆盖前面的,此为重写
...
class Biology: 
    ...
    def play(self,other):
        ...
        other.get_info() # 在不同的子类中重写父类方法之后的调用中就会引用新写的方法 引用不变,方法在变,此为多态
...
* 新式类中有3种设置property属性的方法,根本目的在于简化流程
* 创建`property`属性也可以通过类属性的方式，比如下方的STR.myname
```python
class Foo:
    def func(self):
        pass
...
class STR(str):
    ...
    def delName(self):
        del self.name
    myname = property(fget=getName,fset=setName,fdel=delName,doc="description....") 
    # 通过类属性的方式设置property属性,根本目的是封装
    # 4个参数中前3个分别是获取\改变\删除,对应属性的方法,将该属性的值赋值给新的属性
    # 最后一个参数是字符串，调用该属性的描述信息
...
```
### 导入模块的过程:python找到这个模块然后导入,再定义一个变量名来指向该模块,因此多模块之间沟通需要直接导入模块名来指向模块，而不是`from XX import XX`
* 如果直接import导入，再修改模块文件，即使再import导入一次模块的改变也得不到更新，因为系统会自动避免重新导入同一模块, 若需要更新import导入的模块，需要imp库
```python
from imp import reload
reload(random) # 此时方可得到模块的更新,此法只能更新import 导入的模块
```
* 不推荐使用`from XX import *`的方式，因为函数重名没有任何提示，出问题不好排查
* 因此不管是`import`还是`from XX import X`都只能在导入的值的基础上修改,不能指向一个新的值
```python
from common import RECV_DATA_LIST
from common import HANDLE_FLAG
RECV_DATA_LIST.append(55)  # 修改成功
global HANDLE_FLAG
HANDLE_FLAG = True # 若没有加global则默认修改局部变量，相当于一句赋值语句
```
