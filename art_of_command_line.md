...
- Linux本身就是针对多用户的,没有盘符,只有一个根目录`/`,`/home`文件夹下面的文件夹就是用户的家目录,`/etc`中保存系统配置文件,`/boot`保存linux系统启动的时候用到的一些文件,并且Linux文件(夹)名大小写敏感,`cd ~`或`cd /home`回到家目录的方法,`cd -`在最近两次工作目录之间来回切换, 绝对路径用`/`或者`~`开头, 相对路径 比如`..`表示当前目录的上一个目录
